        ...
        <div class="row">
            <div class="col-sm-6 col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('ProgramStudi.update', $prodi->id)}}">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="kode_cabang">Nama Program Studi</label>
                                <input type="text" class="form-control" id="kode_cabang" name="nama_prodi" placeholder="Input Nama Ormawa" value="{{$prodi->nama_prodi}}">
                            </div>
                            <div class="form-group">
                                ...
                                <select name="ketua_prodi" id="" class="form-control">
                                    ...
                                    @foreach ($dosen as $data)
                                        <option value="{{$data->nama_dosen}}" {{ $prodi->ketua_prodi == $data->nama_dosen ? 'selected' : '' }}>{{$data->nama_dosen}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
